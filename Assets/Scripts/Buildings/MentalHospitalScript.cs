...
public class MentalHospitalScript : MonoBehaviour {
    ...
    void showWindow()
    {
        if (_popUp)
        {
            _windowInfo = GUI.Window(0, _windowInfo, winFunction, _mentalHospital.Name.ToString() + " Lv " + _mentalHospital.Level);
    ...
    void winFunction(int windowID)
    {
        ...
        if (_hero != null)
        {
            ...
            {
                if (action == "RMV")
                    _mentalHospital.Hero = _hero;
                else
                    _mentalHospital.deleteHero();
            }
        }
    }
}
